...
def uniformer_small(pretrained=True, **kwargs):
    model = UniFormer(
        ...
        return_dense=False, mix_token=False, **kwargs)
...
if __name__ == '__main__':
    ...
    save_model = torch.load('/home/hjk/文档/third_model_GCN/pre_train_path/uniformer_large_ls_tl_384.pth')
    model_dict_r = model.state_dict()
    state_dict_r = {k: v for k, v in save_model.items() if k in model_dict_r.keys()}
    model_dict_r.update(state_dict_r)
    model.load_state_dict(model_dict_r)
    res = model(rgb1)
    # print(len(res))
